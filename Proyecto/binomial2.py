...
#Ruben Dario Vargas Yandy
#Proyecto final Analisis y Desarrollo de Algoritmos 2019-1

#Matrices que sirven de tope o sirvieron de limites para iteraciones que usa
...
def getTop(n, i):
        """Esta funcion devuelve el tope mas cercano tanto para low
        como para high para que la busqueda binaria no quede tan a la deriva"""
        top=44721361
        if n<=1000:
                return [2,top2[i]]
...
def ncr(n, r):
        """Funcion que calcula combinatoria de (n, r) de forma eficiente"""
        r = min(r, n-r)
...
def klimit(n):
        """Esta funcion devuelve los valores maximos hacia la derecha
        En que se pueden encontrar los valores menores que 10^15"""
        l=2
        h=120
        mid=(l+h)//2
        midt=mid//2
...
def nc2(n):
        """Funcion para calcular la combinatoria de (n, 2)"""
        return (n-1)*n>>1
def nc3(n):
        """Funcion para calcular la combinatoria de (n, 3)"""
        return (n-2)*(n-1)*n//6

def findInColumn(n, r):
        """Funcion de busqueda binaria que encuentra en que fila n'
        esta el resultado en la columna r, devuelve -1 si no
        lo encuentra"""
        l,h=getTop(n, r)
        l=l-1
        l=max(l, top16[4])
...
def precalc():
        """Esta funcion precalcula los resultados donde es mas probable
        Encontrar resultados y ahorrar en busqedas y calculos posteriores"""
        A={}
        for k in range(4, 28):
...
def solve(n, A):
        """Esta funcion recibe el n a buscar y la matriz precalculada
        Y retorna las tuplas de respuestas solicitadas"""
        if n==2:
                return [(2,1)]
        ans=[]
        if n in A.keys():
